""" Build ``humpy_cytoolz`` with or without Cython.
...
try:
    ...
    version = get_version({'dev_template': '{tag}+{ccount}.g{sha}', 'dirty_template': '{tag}+{ccount}.g{sha}.dirty'})
...
if use_cython and (not has_cython):
    if strict_cython:
        raise RuntimeError('Cython required to build dev version of humpy_cytoolz.')
...
for modname in ['dicttoolz', 'functoolz', 'itertoolz', 'recipes', 'utils']:
    ext_modules.append(Extension('humpy_cytoolz.' + modname.replace('/', '.'), ['humpy_cytoolz/' + modname + suffix]))
if use_cython:
    try:
