...
def test_EqualityHashKey_default_key():
    ...
    set3 = set(data2)
    assert set3 == {(), (1,), EqualityHashDefault(()), EqualityHashDefault((1,))}
    assert len(set3) == 4
...
def test_EqualityHashKey_index_key():
    ...
    assert list(unique(3 * [d1, d2, d3a, d3b], key=EqualityHashFirstname)) == [d1, d2, d3a]
    EqualityHashFirstnameAge = curry(EqualityHashKey, ['firstname', 'age'])
    assert list(unique(3 * [d1, d2, d3a, d3b], key=EqualityHashFirstnameAge)) == [d1, d2, d3a]
    list1 = [0] * 10
    list2 = [0] * 100
    list3a = [1] * 10
    list3b = [1] * 10
    EqualityHash0 = curry(EqualityHashKey, 0)
    assert list(unique(3 * [list1, list2, list3a, list3b], key=EqualityHash0)) == [list1, list2, list3a]
...
def test_unzip():
    ...
    assert _to_lists(unzip([('a', 1), ('b', 2), ('c', 3)])) == [['a', 'b', 'c'], [1, 2, 3]]
