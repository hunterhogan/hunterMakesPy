...
def _merge_sorted_binary_key(seqs: Sequence[Iterable[_CT]], key: UnaryOp[_CT]) -> Iterator[_CT]:
    mid = len(seqs) // 2
    L1 = seqs[:mid]
...
@overload
def get(ind: Sequence[Any], seq: SeqOrMapping[_T], default: _T | NoDefaultType=...) -> tuple[_T, ...]:
    ...

@overload
def get(ind: Any, seq: SeqOrMapping[_T], default: _T | NoDefaultType=...) -> _T:
    ...
...
@overload
def pluck(index: Any, seqs: Iterable[SeqOrMapping[_T]], default: _T | NoDefaultType=...) -> Iterator[_T]:
    ...
...
@overload
def getter(index: list[Any]) -> Callable[[SeqOrMapping[_T]], tuple[_T, ...]]:
    ...
...
def getter(index: Any | list[Any]) -> Callable[[SeqOrMapping[_T]], _T | tuple[_T, ...]]:
...
def join(leftkey: TransformOp[_T, Any], leftseq: Iterable[_T], rightkey: TransformOp[_T, Any], rightseq: Iterable[_T], left_default: _U | NoDefaultType=no_default, right_default: _U | NoDefaultType=no_default) -> Iterator[tuple[_T | _U, _T | _U]]:
...
def diff(*seqs: Iterable[_T], default: _T | NoDefaultType=no_default, key: TransformOp[_T, Any] | None=None) -> Iterator[tuple[_T, ...]]:
    ...
    if not _is_no_default(default):
        iters = cast('Iterator[tuple[_T, ...]]', zip_longest(*all_seqs, fillvalue=default))
    else:
        iters = zip(*all_seqs)
    if key is None:
        for items in iters:
