...
functions also behave the same in `humpy_toolz` and `humpy_cytoolz`.  However, there
...
def test_dicttoolz():
    ...
    assert raises(TypeError, lambda: keymap(None, {1: 2})) or keymap(None, {1: 2}) == {(1,): 2}
    ...
    assert raises((TypeError, AttributeError), lambda: update_in(None, [1, 2], identity))
    ...
    assert raises(TypeError, lambda: valmap(None, {1: 2})) or valmap(None, {1: 2}) == {1: (2,)}
    assert raises((AttributeError, TypeError), lambda: valmap(identity, None))
    tested.append('valmap')
    assert raises(TypeError, lambda: itemmap(None, {1: 2})) or itemmap(None, {1: 2}) == {1: (2,)}
    ...
    s1 = set(tested)
    s2 = set(humpy_cytoolz.dicttoolz.__all__)
...
def test_functoolz():
    ...
    s1 = set(tested)
    s2 = set(humpy_cytoolz.functoolz.__all__)
    assert s1 == s2, '%s not tested for being None-safe' % ', '.join(s2 - s1)
...
def test_itertoolz():
    ...
    tested.append('count')
    assert raises(TypeError, lambda: list(drop(None, [1, 2]))) or list(drop(None, [1, 2])) == [1, 2]
    ...
    assert raises(TypeError, lambda: list(interleave([[1, 2], GenException(ValueError)], pass_exceptions=None)))
    ...
    tested.append('last')
    assert raises(TypeError, lambda: list(mapcat(None, [[1], [2]]))) or list(mapcat(None, [[1], [2]])) == [[1], [2]]
    ...
    assert raises(TypeError, lambda: list(sliding_window(None, [1, 2, 3]))) or list(sliding_window(None, [1, 2, 3])) == []
    assert raises(TypeError, lambda: list(sliding_window(1, None)))
    tested.append('sliding_window')
    assert raises(TypeError, lambda: list(take(None, [1, 2])) == [1, 2]) or list(take(None, [1, 2])) == [1, 2]
    assert raises(TypeError, lambda: list(take(1, None)))
    tested.append('take')
    assert raises(TypeError, lambda: list(tail(None, [1, 2])) == [1, 2]) or list(tail(None, [1, 2])) == [1, 2]
    assert raises(TypeError, lambda: list(tail(1, None)))
    tested.append('tail')
    assert raises(TypeError, lambda: list(take_nth(None, [1, 2]))) or list(take_nth(None, [1, 2])) == [1, 2]
    ...
    s1 = set(tested)
    s2 = set(humpy_cytoolz.itertoolz.__all__)
    assert s1 == s2, '%s not tested for being None-safe' % ', '.join(s2 - s1)

def test_recipes():
    tested = []
    assert raises(TypeError, lambda: countby(None, [1, 2])) or countby(None, [1, 2]) == {(1,): 1, (2,): 1}
    assert raises(TypeError, lambda: countby(identity, None))
    tested.append('countby')
    assert raises(TypeError, lambda: list(partitionby(None, [1, 2]))) or list(partitionby(None, [1, 2])) == [(1,), (2,)]
    assert raises(TypeError, lambda: list(partitionby(identity, None)))
    tested.append('partitionby')
    s1 = set(tested)
    s2 = set(humpy_cytoolz.recipes.__all__)
    assert s1 == s2, '%s not tested for being None-safe' % ', '.join(s2 - s1)
