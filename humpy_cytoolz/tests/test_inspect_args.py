from humpy_cytoolz._signatures import builtins
from humpy_cytoolz.functoolz import (
...
def test_introspect_curry_valid_py3(check_valid=is_valid_args, incomplete=False):
    ...
    f = humpy_cytoolz.curry(make_func('x, y, z=0'))
    assert check_valid(f)
    assert check_valid(f, 1)
    assert check_valid(f, 1, 2)
    ...
    f = humpy_cytoolz.curry(make_func('x, y, z=0'), 1, x=1)
    assert check_valid(f) is False
    assert check_valid(f, z=3) is False
    f = humpy_cytoolz.curry(make_func('x, y, *args, z'))
    assert check_valid(f)
...
def test_introspect_curry_py3():
    f = humpy_cytoolz.curry(make_func(''))
    assert num_required_args(f) == 0
    assert is_arity(0, f)
    assert has_varargs(f) is False
    assert has_keywords(f) is False
    f = humpy_cytoolz.curry(make_func('x'))
    assert num_required_args(f) == 0
    ...
    assert has_varargs(f) is False
    assert has_keywords(f)
    f = humpy_cytoolz.curry(make_func('*args, **kwargs'))
    assert num_required_args(f) == 0
    assert has_varargs(f)
    assert has_keywords(f)

def test_introspect_builtin_modules():
    mods = [builtins, functools, itertools, operator, humpy_cytoolz, humpy_cytoolz.functoolz, humpy_cytoolz.itertoolz, humpy_cytoolz.dicttoolz, humpy_cytoolz.recipes]
    denylist = set()
...
def test_inspect_signature_property():
    ...
    assert sig == inspect.Signature(parameters=[inspect.Parameter('addx', inspect.Parameter.POSITIONAL_OR_KEYWORD), inspect.Parameter('x', inspect.Parameter.POSITIONAL_OR_KEYWORD)])
